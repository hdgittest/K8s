...
func (c *Controller) syncStatus(serviceCIDR *networkingapiv1.ServiceCIDR) {
	...
	if !serviceCIDR.GetDeletionTimestamp().IsZero() {
		klog.V(4).Infof("ServiceCIDR %s is being deleted, skipping status sync", serviceCIDR.Name)
		return
	}
	...
	// Current Ready=False.
	if currentReadyCondition != nil && currentReadyCondition.Status == metav1.ConditionFalse {
		...
		// Don't try to overwrite Ready=False set by another component.
		return
	}

	// Current Ready=True and config matches, nothing to do.
	if currentReadyCondition != nil && currentReadyCondition.Status == metav1.ConditionTrue {
		klog.V(4).Infof("ServiceCIDR %s is Ready and configuration matches. No status update needed.", serviceCIDR.Name)
		return
	}

	// No condition set and ServiceCIDR matches this apiserver configuration, set condition to True
	if currentReadyCondition == nil || currentReadyCondition.Status == metav1.ConditionUnknown {
