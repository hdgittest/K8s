...
var (
	...
	pendingNotifications = k8smetrics.NewGaugeVec(&k8smetrics.GaugeOpts{
		...
		StabilityLevel: k8smetrics.ALPHA,
	}, []string{"name", "resource_type", "handler_name"})
...
func (informerMetricsProvider) NewStoredItemMetric(name string, resourceType string) cache.GaugeMetric {
	return storedItems.WithLabelValues(name, resourceType)
}

func (informerMetricsProvider) NewQueuedItemMetric(name string, resourceType string) cache.GaugeMetric {
	return queuedItems.WithLabelValues(name, resourceType)
}

func (informerMetricsProvider) NewPendingNotificationsMetric(name string, resourceType string, handlerName string) cache.GaugeMetric {
	return pendingNotifications.WithLabelValues(name, resourceType, handlerName)
}

func (informerMetricsProvider) NewRingGrowingMetric(name string, resourceType string, handlerName string) cache.GaugeMetric {
...
func (informerMetricsProvider) NewPrcoessDurationMetric(name string, resourceType string, handlerName string) cache.HistogramMetric {
	return eventProcessDuration.WithLabelValues(name, resourceType, handlerName)
}
