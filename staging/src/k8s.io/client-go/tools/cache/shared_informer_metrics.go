...
type InformerMetricsProvider interface {
	...
	NewStoredItemMetric(informerName string, resourceType string) GaugeMetric
	// NewQueuedItemMetric returns metric for total number of item in queue.
	NewQueuedItemMetric(informerName string, resourceType string) GaugeMetric
	// NewPendingNotificationsMetric returns metric for pending notifications.
	NewPendingNotificationsMetric(informerName string, resourceType string, handlerName string) GaugeMetric
	// NewRingGrowingMetric returns metric for ring buffer.
	NewRingGrowingMetric(informerName string, resourceType string, handlerName string) GaugeMetric
	// NewPrcoessDurationMetric returns metric for event processing duration.
	NewPrcoessDurationMetric(informerName string, resourceType string, handlerName string) HistogramMetric
}

type noopInformerMetricsProvider struct{}

func (noopInformerMetricsProvider) NewStoredItemMetric(informerName string, resourceType string) GaugeMetric {
	return noopMetric{}
}
func (noopInformerMetricsProvider) NewQueuedItemMetric(informerName string, resourceType string) GaugeMetric {
	return noopMetric{}
}
func (noopInformerMetricsProvider) NewPendingNotificationsMetric(informerName string, resourceType string, handlerName string) GaugeMetric {
	return noopMetric{}
}
func (noopInformerMetricsProvider) NewRingGrowingMetric(informerName string, resourceType string, handlerName string) GaugeMetric {
	return noopMetric{}
}
func (noopInformerMetricsProvider) NewPrcoessDurationMetric(informerName string, resourceType string, handlerName string) HistogramMetric {
	return noopMetric{}
}
...
func newInformerMetrics(identifier InformerIdentifier) *informerMetrics {
	var ret *informerMetrics
	if identifier.Name == "" || identifier.Type == "" {
		return ret
	}

	return &informerMetrics{
		clock:               &clock.RealClock{},
		numberOfStoredItem:  sharedInformerMetricsFactory.getMetricsProvider().NewStoredItemMetric(identifier.Name, identifier.Type),
		numberOfQueuedItem:  sharedInformerMetricsFactory.getMetricsProvider().NewQueuedItemMetric(identifier.Name, identifier.Type),
